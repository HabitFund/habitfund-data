...
4. 설정:
   - **Note**: `HabitFund Data Sync` (원하는 이름)
...
처음 함수를 실행하거나 트리거를 저장하면 권한 요청이 나타납니다:

#### "Google hasn't verified this app" 경고가 나타나면:
1. **고급** (Advanced) 클릭
2. **"{프로젝트 이름}(으)로 이동 (안전하지 않음)"** (Go to {project name} (unsafe)) 클릭
3. **허용** (Allow) 클릭

> ⚠️ 이 경고는 Apps Script가 외부 API(GitHub API)에 접근하기 때문에 나타납니다.  
> 본인이 작성한 스크립트이므로 안전하게 진행할 수 있습니다.
